...
data1 = pd.read_csv('Lund_hotels_overview.csv', names = ['name', 'url', 'review count',
                                                         'aggregating rating value', 'price range',
                                                         'address', 'postcode', 'category'])

data2= pd.read_csv('25_hotels_together_with-ID-name.csv',
                   names = ['poi_name', 'poi_id','rating author', 'rating date','rating score',
                            'rating title', 'text','response title', 'response date',
                            'response text', 'date of stay', 'review language', 'review count by language'])
...
data2 = data2[['poi_name', 'poi_id', 'total review count', 'average rating score', 'price',
               'address', 'postcode', 'category', 'review language', 'review count by language',
                            'rating author', 'rating date', 'rating score', 'date of stay',
                            'rating title', 'text','response title', 'response date', 'response text']]
print(data2.head(1))

data2.to_csv('./' + '25_hotels_lund_add_price_address_etc' +'.csv', mode = 'w', index= False)
