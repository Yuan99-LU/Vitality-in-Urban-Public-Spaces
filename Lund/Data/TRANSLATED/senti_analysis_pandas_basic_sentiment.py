...
for file_name in file_names[0:2]:
    ...
    data['basic_sentiment_text'] = np.array([ analyze_basic_sentiment(text) for text in data['eng_text'] ])
    data['basic_sentiment_title'] = np.array([ analyze_basic_sentiment(text) for text in data['eng_title'] ])
    ...
    #
    print(data.head())
    data.to_csv("senti_analysis_output/" + file_name[0:-4] + "basic_senti_analysis.csv", index = False)
